...
def run_worker(directory: Path, emit, quiet_window: float = 0.5,
    ...
    def request_augmentation(received_at):
        if sampler is None or not bursts.reserve_augmentation():
            return False
        key = id(received_at)
        trigger = ProgressiveTrigger(received_at)
...
def _process_bursts(bursts, emit, on_error=None) -> None:
    while True:
        ...
        if item is None:
            return
        if isinstance(item, BurstWork):
            _process_progressive_work(item, emit, on_error, bursts)
            continue
        paths, received_at, *timing = item
        try:
...
def _process_progressive_work(work: BurstWork, emit, on_error, bursts) -> None:
    ...
    if work.kind == "augmentation":
        ...
        if waiting_for_primary:
            return
        _finalize_progressive_trigger(trigger, emit, on_error)
        return
...
def _finalize_progressive_trigger(trigger: ProgressiveTrigger, emit, on_error) -> None:
    with trigger._lock:
    ...
    try:
        emit(combined, received_at, *timing, **options)
    except Exception as error:
        if on_error is not None:
            on_error(combined, error, received_at)
    finally:
        _remove_uploads(combined)
        trigger.release_once()


def _abandon_progressive_trigger(
    trigger: ProgressiveTrigger, bursts: BoundedBurstQueue
) -> None:
    with trigger._lock:
        if trigger.abandoned:
            return
...
def _discard_burst_work(item, bursts: BoundedBurstQueue) -> None:
    work_item = item.item if isinstance(item, BurstWork) else item
    _remove_uploads(work_item[0])
    if isinstance(item, BurstWork):
        _abandon_progressive_trigger(item.trigger, bursts)
