...
Effective approximately 2026-08-19 18:10 Europe/Dublin, the approved baseline
uses the current post-reposition camera angle, a full-width horizontal line,
vehicle-only detection, sensitivity 80, and both crossing directions. This is a
frozen evidence baseline for controlled comparison, not a configuration proven
to be optimal.

Any variant must be evaluated against this baseline for inbound recall, false
events and night precision, trigger-to-decision latency, and gate
acknowledgement. Do not combine setting changes in a way that prevents their
effects from being attributed during validation.
...
2.25-second wall-clock deadline. They are written temporarily to an ignored
owner-only `.reolink-snapshots` directory beside the upload root and validated
as bounded JPEGs. The first FTP image enters recognition on its normal quiet
window and establishes one durable trigger identity. Authorizing FTP results
finalize immediately; only an improvable denial remains provisional while the
snapshots reuse that identity and its single actuation claim. Generated files
cannot recursively request another sample, and snapshot sampling has no relay
path. Failure, timeout, shutdown, or unavailable configuration terminally
